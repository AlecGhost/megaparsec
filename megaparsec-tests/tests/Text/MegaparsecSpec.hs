...
spec = do
  ...
  describe "ParsecT IsString instance" $ do
    ...
    it "can handle Unicode" $ do
      let r = "פּאַרסער 解析器" :: BS.ByteString
      ...
      context "with two strings" $ do
        ...
        context "when stream does not begin with either string" $
          it "signals correct error message" $
            property $ \s0 s1 s ->
              not (s0 `isPrefixOf` s) && not (s1 `isPrefixOf` s) ==> do
              ...
              b /= c ==> do
                let p :: (MonadParsec Void String m) => m (Either (ParseError String Void) Char)
                    p =
                      withRecovery
                        (\e -> Left e <$ string (c : as))
                        (Right <$> char a <* many (char b) <* char b)
                        <* empty
                    s = a : c : as
      ...
      it "works in complex situations too" $
        property $ \(NonNegative a) (NonNegative b) (NonNegative c) -> do
          let p :: (MonadParsec Void String m) => m (Either (ParseError String Void) String)
              ...
              z = flip shouldParse
              r
                | a == 0 && b == 0 && c == 0 = f (err 0 (ueof <> etok 'a'))
    ...
    describe "parseMaybe" $
      it "returns result on success and Nothing on failure" $
        property $ \s s' -> do
          ...
          parseMaybe p s
            `shouldBe` if s == s' then Just s else Nothing
      ...
      it "generally works" $
        property $ \pre ch1 ch2 -> do
          let s1 = pre : [ch1]
              ...
              s = [pre]
          prs (runReaderT p (s1, s2)) s
            `shouldFailWith` err 1 (ueof <> etok ch1 <> etok ch2)
        ...
        property $ \(NonNegative a) (NonNegative b) (NonNegative c) -> do
          ...
          if b > 0 || c > 0
            then prs (runReaderT p 'a') s `shouldParse` replicate a 'a'
              ...
              prs (runReaderT p 'a') s
                `shouldFailWith` err a (ueof <> etok 'a')
  ...
  describe "MonadParsec instance of lazy RWST" $ do
    describe "label" $
      it "allows to access reader context and state inside it" $
        property $ \r s -> do
          ...
          prs (L.runRWST p (r :: Int) (s :: Int)) ""
            `shouldParse` ((r, s), s, mempty :: [Int])
    ...
    describe "lookAhead" $ do
      ...
      it "discards what writer tells inside it" $
        property $ \w -> do
          let p = lookAhead (L.tell [w])
          prs (L.runRWST p (0 :: Int) (0 :: Int)) ""
            `shouldParse` ((), 0, mempty :: [Int])
      it "does not allow to influence state outside it" $
        property $ \s0 s1 ->
          (s0 /= s1) ==> do
            let p = lookAhead (L.put s1)
            prs (L.runRWST p (0 :: Int) (s0 :: Int)) ""
              `shouldParse` ((), s0, mempty :: [Int])
  ...
  describe "MonadParsec instance of strict RWST" $ do
    ...
    describe "lookAhead" $ do
      ...
      it "discards what writer tells inside it" $
        property $ \w -> do
          let p = lookAhead (S.tell [w])
          prs (S.runRWST p (0 :: Int) (0 :: Int)) ""
            `shouldParse` ((), 0, mempty :: [Int])
      it "does not allow to influence state outside it" $
        property $ \s0 s1 ->
          (s0 /= s1) ==> do
            let p = lookAhead (S.put s1)
            prs (S.runRWST p (0 :: Int) (s0 :: Int)) ""
              `shouldParse` ((), s0, mempty :: [Int])
